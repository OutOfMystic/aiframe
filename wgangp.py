...
s_path = os.path.abspath(__file__).split('\\')[:-1]
sys.path.insert(0, os.path.dirname(s_path))
...
class WGAN(keras.Model):
    ...
    def train(self, batch_size=32, project='Project', packs=3, load=False,
        ...
        while True:
            idx = np.random.randint(-1, shape, batch_size)
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def __init__(self, model, img_shape, img_table=(5, 5), name='Project',
        ...
        self.project_path = f'projects\\{self.name}\\'
    ...
    def plot_history(self):
        ...
        pyplot.legend()
        pyplot.savefig(self.project_path + 'plot_stats.png', dpi=500)
        pyplot.close()
        print('Hp', end='')
...
def get_discriminator_model(img_shape):
    ...
    x = conv_block(img_input, 64, layers.LeakyReLU(0.2), 3, 2, use_bias=True,
                   use_dropout=False, drop_value=0.3)
    x = conv_block(x, 64, layers.LeakyReLU(0.2), 3, 2, use_bias=True,
                   use_dropout=True, drop_value=0.1)
    x = conv_block(x, 64, layers.LeakyReLU(0.2), 3, 2, use_bias=True,
                   use_dropout=True, drop_value=0.1)
    x = conv_block(x, 96, layers.LeakyReLU(0.2), 3, 2, use_bias=True,
                   use_dropout=True, drop_value=0.1)
    x = conv_block(x, 128, layers.LeakyReLU(0.2), 3, 2, use_bias=True,
                   use_dropout=False, drop_value=0.3)
...
def get_generator_model(noise_dim, start_dense, dimensions):
    ...
    x = upsample_block(x, 164, layers.LeakyReLU(0.2), use_bias=False, use_bn=True)
    x = upsample_block(x, 128, layers.LeakyReLU(0.2), use_bias=False, use_bn=True)
    x = upsample_block(x, 96, layers.LeakyReLU(0.2), use_bias=False, use_bn=True)
    x = upsample_block(x, 64, layers.LeakyReLU(0.2), use_bias=False, use_bn=True)
    x = upsample_block(x, dimensions, layers.Activation("tanh"), kernel_size=5,
                       strides=(1, 1), use_bias=False, use_bn=True)
...
if __name__ == '__main__':
    ...
    g_model = get_generator_model(noise_dim, start_dense, img_shape[-1])
    g_model.summary()
    ...
    cbk = GANMonitor(wgan, img_shape, img_table=img_table, name=project, latent_dim=noise_dim)
    #wgan.fit(train_images, batch_size=batch_size, epochs=epochs, callbacks=[cbk])
    wgan.train(batch_size=batch_size, project=project, packs=packs, load=load, cbk=cbk)